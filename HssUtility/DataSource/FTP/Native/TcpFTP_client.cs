...
namespace HssUtility.DataSource.FTP.Native
{
    public class TcpFTP_client
    {
        ...
        /// Default contructor
        /// </summary>
        public TcpFTP_client()
        {
        }

        public TcpFTP_client(string server, string username, string password)
        {
            this.server = server;
        ...
        public TcpFTP_client(string server, string username, string password, int timeoutSeconds, int port)
        {
            this.server = server;
            this.username = username;
            this.password = password;
        ...
        public bool Login()
        {
            ...
            try
            {
                ...
                if (!IPAddress.TryParse(this.server, out addr)) addr = Dns.GetHostEntry(this.server).AddressList[0];

                ep = new IPEndPoint(addr, this.port);

                this.clientSocket.Connect(ep);
                /*May need to wait some while*/
        ...
        public long Download(string remFileName, string locFileName)
        {
        ...
        /// <param name="locFileName"></param>
        /// <param name="resume"></param>
        public long Download(string remFileName, string locFileName, Boolean resume)
        {
            if (!this.loggedin) this.Login();

            this.BinaryMode = true;

            Console.WriteLine("--> Downloading file " + remFileName + " from " + server + "/" + remotePath);            
            ...
            Socket cSocket = this.createDataSocket();/*Create pasive mode socket*/
        ...
        private string readLine()
        {
            ...
            Console.WriteLine("--> Server said: " + this.message.Replace("\r", "").Replace("\n", ""));
        ...
        private Socket createDataSocket()
        {
            ...
            string ipAddr_str = parts[0] + "." + parts[1] + "." + parts[2] + "." + parts[3];
            ...
            try
            {
                ...
                if (!IPAddress.TryParse(ipAddr_str, out IPAddress ip_addr)) ip_addr = Dns.GetHostEntry(ipAddr_str).AddressList[0];
                ep = new IPEndPoint(ip_addr, port);
